...
export default function MyServicesPage() {
  ...
  return (
    <DashboardLayout
      title="My Services"
      subtitle="Provider Dashboard • Manage your professional offerings."
    >
          ...
          {loading ? (
            ...
            <div className="text-center py-20 bg-white rounded-xl border border-gray-100 border-dashed animate-fade-in shadow-sm">
              ...
              <p className="mt-2 text-gray-500 mb-6 max-w-sm mx-auto">
                Get started by creating your first service listing so availers
                can find you.
              </p>
              <Link
                    ...
                    <div className="border-t border-gray-100 pt-4 flex items-center justify-between mt-auto">
                      ...
                      <div className="flex items-center gap-2">
                        <Link
                          ...
                          className="text-brand-600 bg-brand-50 hover:bg-brand-100 font-medium px-4 py-2 rounded-lg text-sm transition-colors border border-brand-100 flex items-center gap-2 relative"
                        >
                          Applicants
                          {service._count?.applications > 0 && (
                            <span className="absolute -top-2 -right-2 bg-red-500 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full min-w-[20px] text-center">
                              {service._count.applications}
                            </span>
                          )}
                        </Link>
                        <Link
                          href={`/provider/services/${service.id}/edit`}
                          className="text-gray-500 hover:text-brand-600 bg-gray-50 hover:bg-brand-50 p-2 rounded-lg transition-colors border border-gray-100 flex items-center justify-center h-[38px] w-[38px]"
                          title="Edit Service"
                        >
                          <svg
                            className="w-4 h-4"
