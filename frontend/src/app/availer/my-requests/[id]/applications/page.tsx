...
export default function RequestApplicationsPage() {
  ...
  const fetchData = async () => {
    try {
      // Since there's no single endpoint to get a request by ID easily mapped right now if it's my list,
      // let's fetch my requests and find it. This is a bit hacky but works for demo scale.
      const res = await axios.get("/avail-requests/my");
      const req = res.data.data.find((r: any) => r.id === id);
      setRequestItem(req);

      if (req) {
        const appRes = await axios.get(`/avail-requests/${id}/applications`);
  ...
  const handleAccept = async (applicationId: string) => {
    if (
      !confirm(
        "Are you sure you want to accept this provider? All other applications will be rejected and the request will be reserved.",
      )
    )
      return;
    ...
    try {
      await axios.post(
        `/avail-requests/${id}/applications/${applicationId}/accept`,
      );
      router.push("/availer/my-requests");
    } catch (err: any) {
      ...
      {loading ? (
        <div className="animate-pulse space-y-4 max-w-4xl">
          {[1, 2].map((i) => (
            <div
              key={i}
              className="h-40 bg-white/50 rounded-xl border-dashed"
            ></div>
          ))}
        </div>
      ) : applications.length === 0 ? (
        <div className="text-center py-20 bg-white rounded-xl border border-gray-100 max-w-4xl shadow-sm">
          <svg
          ...
          <p className="mt-1 text-gray-500">
            Wait for professionals to apply to your request.
          </p>
        </div>
      ) : (
                ...
                <div className="flex-1">
                  <div className="flex justify-between items-start mb-4">
                    <div className="flex items-center gap-3">
                      <div className="w-12 h-12 rounded-full bg-accent-100 flex items-center justify-center text-accent-700 font-bold text-lg shadow-sm">
                  ...
                  <div className="bg-gray-50/80 p-4 rounded-lg border border-gray-100 mb-4 whitespace-pre-wrap italic text-sm text-gray-700 relative">
                    ...
                    <span className="relative z-10 pl-6 block">{`"${app.note}"`}</span>
                  </div>
